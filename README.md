...
**Quiet hours** Хранится строками. Например `22:00–08:00` + `Europe/Berlin` смещенийй по времени не делаем. В некоторых странах, меняется время на летнее и зимнее. храним локальное время пользователя и зону. UTC-момент запроса конвертируется в локальное время зоны через `Intl.DateTimeFormat`. Интервалы через полночь поддержаны.
